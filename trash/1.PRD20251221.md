...
## 5.1 核心套件與依賴 (Core Dependencies)
*   **Validation & Security**:
    *   `zod`: 嚴格的資料格式驗證 (Schema Validation)，與 tRPC 深度整合。
    *   `dompurify`: 過濾編輯器 (TipTap/Markdown) 輸出的 HTML，防止 XSS 攻擊。
*   **Logging & Error Tracking**:
    *   `pino`: 高效能結構化日誌記錄 (Structured Logging)。
    *   `sentry`: 生產環境錯誤追蹤與效能監控。
    *   `posthog-js`: 產品分析與使用者行為追蹤 (Audit Logs)。
*   **Testing**:
    *   `vitest`: 極速單元測試 (Unit Testing)。
    *   `playwright`: 可靠的端對端整合測試 (E2E Testing)。
    *   `@testing-library/react`: 元件行為測試。
*   **Utilities**:
    *   `date-fns`: 輕量級且現代的時間處理庫。
    *   `i18next` (`next-i18next`): 完整的國際化解決方案。
    *   `lucide-react`: 統一且美觀的 SVG 圖標庫。
    *   `clsx` / `tailwind-merge`: 動態 Tailwind Class 管理。

## 5.2 介面與互動 (UI & Styling)
*   **Styling System**:
    *   `tailwindcss`: Utility-first CSS 框架。
    *   `tailwindcss-animate`: 動畫工具擴充。
*   **Component Primitives**:
    *   `radix-ui`: 無樣式、無障礙 (A11y) 的底層元件庫 (Dialog, Popover, Switch...)。
    *   `shadcn/ui`: 基於 Radix UI 與 Tailwind 的現代化元件集合 (Copy & Paste base)。
*   **Motion & Interaction**:
    *   `framer-motion`: 複雜互動與頁面轉場動畫 (Animation)。
    *   `cmdk`: 用於實作全域搜尋與 Command Palette。
    *   `vaul`: 用於行動裝置友善的抽屜式視窗 (Drawer)。
    *   `sonner`: 簡約美觀的 Toast 通知。
*   **Visualization**:
    *   `recharts`: 用於繪製儀表板圖表 (Line Chart, Heatmap)。

## 5.3 狀態與資料處理 (State & Logic)
*   **State Management**:
    *   `zustand`: 輕量級全域狀態管理 (用於 Sidebar 狀態、各種 Modal 開關)。
    *   `nuqs`: URL 參數狀態管理 (Type-safe search params)，讓篩選條件可被分享與書籤化。
    *   `react-hook-form`: 強大的表單處理庫 (Login, Create Project)。
    *   `@hookform/resolvers`: 連接 Zod 與 React Hook Form。
*   **Interaction Utilities**:
    *   `@dnd-kit/core`: 現代化的拖拉 (Drag & Drop) 解決方案 (用於 Workbench 任務排序)。
    *   `next-themes`: 完美支援 Next.js 的 Dark Mode 切換。
*   **Backend & Parsing**:
    *   `cheerio`: Server 端 HTML 解析與爬蟲 (用於 Quick Capture 的 URL Metadata 抓取)。
...
3.  **依賴安裝**: 依據 `Sec 5` 安裝所有核心套件，重點包含 `shadcn/ui`, `zustand`, `zod`, `pino` 等。
...
2.  **頁面實作清單**:
    *   **Auth**: Login / Register 頁面 (整合 `react-hook-form` 驗證 UI)。
    *   **Landing / Dashboard**: 總覽儀表板 (使用 `recharts` 繪圖)。
    *   **Inbox**: 資源清單 (Inbox List) 與 快速捕捉 (Quick Capture Modal)。
    *   **Resources**: 資源庫清單 (Resource Library) 與 內容編輯頁 (TipTap Editor)。
    ...
    *   **Settings**: 系統設定與標籤管理。
3.  **互動與狀態實作**: 
    *   使用 `dnd-kit` 實作 Workbench 任務排序。
    *   使用 `nuqs` 實作列表頁的 URL 篩選狀態同步。
    *   使用 `sonner` 實作全域 Toast 通知。
4.  **風格微調**: 依據 `4.design_guide.md` 調整 Typography, Colors, Spacing 與 Dark Mode (`next-themes`)。
...
2.  **tRPC Router 實作**: 依據 Data Model 實作 CRUD。
    *   特別實作：Quick Capture 的 URL 解析邏輯 (整合 `cheerio` 爬蟲)。
...
**目標**：確保系統穩定，資料與存取權限無誤。
1.  **測試策略執行**:
    *   **Unit Test**: 使用 `vitest` 測試 Utils (如日期計算) 與 Hooks。
    *   **E2E Test**: 使用 `playwright` 錄製並測試關鍵路徑 (註冊 -> 新增 Note -> 檢查 Dashboard)。
2.  **RLS 檢查**: 嚴格檢查 Supabase Row Level Security (RLS) 規則，確保使用者只能存取自己的資料。
3.  **Input Validation**: 強化 Zod Schema 驗證，防止 XSS (`dompurify`) 與惡意輸入。

## Phase 5: 部署與初期驗證 (Deployment & Alpha)
**目標**：將服務推上雲端，進行真實環境測試。
1.  **監控配置**: 整合 `sentry` (錯誤追蹤) 與 `posthog` (基礎分析) 於生產環境。
2.  **Vercel 部署**: 設定 GitHub Actions CI/CD，連結 Vercel Project，確保 Push 即部署。
3.  **個人狗食測試 (Dogfooding)**: 開發者本人開始遷移真實資料至 Kernel，進行至少 3-7 天的日常使用。
4.  **Bug Bash**: 根據個人使用經驗，快速修復 UI UX 痛點與效能問題。
